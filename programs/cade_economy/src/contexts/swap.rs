...
impl<'info> Swap<'info> {
    pub fn swap(&mut self, amount: u64, expiration: i64, is_usdc: bool) -> Result<()> {
        // assert_not_locked!(self.config.locked);
        // assert_not_expired!(expiration);
        // assert_non_zero!([amount]);

        self.deposit_token(amount, is_usdc);
        self.send_lp_token(amount * 10);

        Ok(())
    }

    pub fn deposit_token(&mut self, amount: u64, is_usdc: bool) -> Result<()> {
        if is_usdc {
